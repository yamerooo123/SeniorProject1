...
features = ['Brand', 'Model', 'Type', 'Gender', 'Color', 'Material']
...
def combined_features(row):
    return row['Brand'] + " " + row['Model'] + " " + row['Type'] + " " + row['Gender'] + " " + row['Color'] + " " + row['Material']
...
def get_similar_shoes(shoes_input, cosine_sim, shoes_dataset, top_n=5):
    search_results = shoes_dataset[shoes_dataset["Model"].str.contains(shoes_input, case=False)].index
    if len(search_results) == 0:
        print("No matches found for:", shoes_input)
        return []
